...
export default {
  ...
  methods: {
    async fetchPerson() {
      try {
        const response = await axios.get(`/persons/${this.personName}`); // Proxy ayarı kullanıldığında sadece yol yazılır
    ...
    async populateRelatives(ids, type) {
      ...
      for (const id of ids) {
        try {
          const response = await axios.get(`/persons/${id}`); // Proxy ayarı kullanıldığında sadece yol yazılır
